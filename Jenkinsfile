node('master') {
     withCredentials([
        ...
        withEnv([
            ...
            try {
                stage('run tests') {
                    bat 'docker run --rm --network=host -e APP_HOST=localhost -e APP_PORT=' + env.APP_PORT + ' -e LOGGING_LEVEL=' + env.LOGGING_LEVEL + ' -v "' + pwd() + '/tests/:/app/tests/" -v common:/app/common/ tests'
